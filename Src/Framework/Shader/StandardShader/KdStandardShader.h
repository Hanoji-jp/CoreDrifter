...
public:
	...
	// 輪郭を描いている最中か。
	//
	// 材質の色は普段どおり「モデルの色 × 指定した色」で掛け合わせるが、
	// 輪郭のときだけは掛けずに、指定した色をそのまま出す。
	//
	// 掛けると、モデルの色が黒いときに何色を指定しても黒になる。
	// 外から持ってきたモデルは黒い材質を持っていることが珍しくなく、
	// 「輪郭は出るが色が乗らない」という状態になる。
	//
	// 輪郭は「誰の車か」を見分けるための線なので、
	// モデルによって色が変わってしまうと役に立たない。
