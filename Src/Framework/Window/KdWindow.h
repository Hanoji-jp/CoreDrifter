...
public:
	...
	// マウスの生の移動量を取得 (Raw Input。OSのマウスアクセラレーションに依存しない)
	POINT GetMouseRawMove() const { return { m_mouseRawMoveX, m_mouseRawMoveY }; }
	// インスタンスハンドル取得
	...
	// ウィンドウ作成
	bool Create(int clientWidth, int clientHeight, std::string_view titleName, std::string_view windowClassName, bool isBorderlessWindow = false);
...
private:
	...
	// マウスの生の移動量(1フレーム分、Raw Inputで蓄積)
	int		m_mouseRawMoveX = 0;
	int		m_mouseRawMoveY = 0;
