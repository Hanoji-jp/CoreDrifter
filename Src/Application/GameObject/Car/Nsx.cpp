...
Nsx::Nsx()
{
	...
	// 車輪1つを原点へ置いたものに分けてある
	m_bodyPath   = "Asset/Data/nsx/nsx_body.gltf";
	m_wheelPath  = "Asset/Data/nsx/nsx_wheel.gltf";
	m_tuningName = "NSX Tuning";
	m_saveKey    = "Nsx";
	...
	m_smokeColor  = Math::Vector3(1.00f, 0.55f, 0.30f);   // 明るい橙
	m_smokeColorB = Math::Vector3(0.78f, 0.20f, 0.24f);   // 深い赤(彩度は保つ)

	// 性能ステータス(NSX / MR・自然吸気)
	m_enginePower = 11.0f;   // シルビアよりわずかに穏やか
	m_brakePower  = 19.0f;   // 車重が軽く、止まりはよい
	m_maxSpeed    = 68.0f;   // 最高速は伸びる(≈245km/h)
	m_drag        = 0.065f;
